...
const octokit = new Octokit({ auth: "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" });
...
async function loadPlugin() {
    ...
    if (global.coreconfig.main_bot.developMode) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (!fs.lstatSync(check).isDirectory()) {
                try {
                    let pluginInfo = await evelStringInit(fs.readFileSync(path.join(__dirname, "..", "..", "plugins", list[i])).toString());
    ...
    for (i = 0; i < name.length; i++) {
        try {
            ...
            let pluginInfo = await evelStringInit(func);
